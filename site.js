...
app.get("/pair", requireLogin, async (req, res) => {
  ...
  try {
    // NOTE: pair.js now clears the stale code itself, right before
    // requesting a new one.  Do NOT wipe it here — wiping before
    // startpairing() returns races with pair.js writing the fresh code,
    // which causes the frontend to see {} even after the code is ready.
    ...
    console.error(`❌ Pairing failed for ${number}:`, e.message);
